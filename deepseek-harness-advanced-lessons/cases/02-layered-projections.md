...
1. 本课最重要的单文件阅读：`sed -n '1,40p' packages/client/runtime/README.md`。
2. higher-seq-wins 的测试证据：`grep -rn "higher-seq-wins" packages/client/runtime/tests/*.spec.ts | head -3`——用例名就是并发协议。
3. 乐观更新与推送帧同 seq 幂等：找 `rename` 的测试用例，读它如何断言"重放同一 seq 是 no-op"。
