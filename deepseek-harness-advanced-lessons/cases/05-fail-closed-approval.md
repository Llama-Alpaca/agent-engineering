...
## 动手验证（在真实 checkout 里，实测）

**Break-it 全流程（作者逐条实测）**——上游测试就是这个决策的执法者，删掉决策、看谁变红：

```bash
# 1. 基线：38 个测试全绿
pnpm vitest run packages/interaction/user-approval --reporter=dot
#       Tests  38 passed (38)

# 2. 删掉 fail-closed 归一化（rogue 返回值 → 'unavailable' 那一行）
sed -i '' 's|outcome => OUTCOMES.includes(outcome) ? outcome : .unavailable.,|outcome => outcome,|' \
  packages/interaction/user-approval/src/index.ts

# 3. 重跑：恰好一个测试变红
pnpm vitest run packages/interaction/user-approval --reporter=verbose
#   × ApprovalService.request > normalizes a rogue non-vocabulary answer to unavailable
#     → expected 'yolo' to be 'unavailable'    # 测试连 rogue 值都起好了名字

# 4. 还原，回到 38 绿
git checkout -- packages/interaction/user-approval/src/index.ts
```

注意第 3 步的输出：失败信息 `expected 'yolo' to be 'unavailable'` 把决策理由演示得干干净净——**未归一的 rogue 值会直接漏进调用方的 closed-union switch**。这就是"fail-closed 靠归一实现"的全部意义。
