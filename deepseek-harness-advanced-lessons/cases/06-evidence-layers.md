...
1. 分层纲领一页：`sed -n '1,50p' docs/testing.md`——把"绝不证明什么"抄下来。
2. lane 与 CI 的对应：`grep -n '"test' package.json | head -10` 对照 `.github/workflows/ci.yml`。
3. replay 只读纪律：`grep -n "DSH_SNAPSHOT\|replay" .github/workflows/ci.yml | head -5`——CI 永远不写基准。
4. （重实验，可选）跑一条 keyless lane 感受"没 key 也全绿"：`pnpm test`（unit lane，全仓需要几分钟）。
