...
1. 决策宣言只有一行注释：`grep -n "without assigning" packages/sdk/server/src/server.ts`。
2. 读客户端归因循环：`sed -n '/while (true)/,/^  }/p' packages/sdk/client/src/api.ts | head -20`——左右端点都是日志事件。
3. 双语言十字检查：`grep -n "_is_inbox_receipt" -A 6 python/sdk/src/deepseek_harness/api.py`，对照 TS 侧 `isInboxReceipt`——两份独立实现是彼此的合规测试。
