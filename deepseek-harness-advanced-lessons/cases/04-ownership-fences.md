...
jobs 套件基线（作者实测，与 preset 合跑 194 绿）：

```
pnpm vitest run packages/jobs/jobs-local --reporter=dot
```

1. 边界格言：`grep -n "Ids are predictable" packages/jobs/jobs-local/src/index.ts`；诚实声明：`grep -n "records die with the harness process" packages/jobs/jobs-local/README.md`。
2. first-wins 的执法测试：`grep -n "First-wins preserves a teardown force-failure" packages/jobs/jobs-local/src/index.ts` 找到注释，再在 tests 里找同名用例跑它。
3. 破坏实验（建议）：删掉 `assertAccess` 的 owner 校验重跑套件——owner fence 的用例立刻变红。做完还原。
