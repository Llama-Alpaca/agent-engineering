...
if (anchors.anchors.length < 40) throw new Error(`expected >=40 course anchors, got ${anchors.anchors.length}`)
...
const fake = mkdtempSync(join(tmpdir(), 'dsh-course13-anchors-'))
try {
...
console.log('course13 checks: ok')
