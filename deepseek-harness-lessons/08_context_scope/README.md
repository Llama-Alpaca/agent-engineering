...
这是映射课：概念课程十一已经讲过"为什么"，本课只回答"在哪、怎么实现、归谁所有"。**注意本课的读法与前几课不同**：不逐文件精读，而是把一张映射表逐行对着源码验证。
...
1. **机制不进循环。** 压缩、瘦身、外置、技能全部是挂在既有扩展点（surface op、session event、ctx service）上的插件；`ReactLoopAgent` 对它们一无所知。这是 L01"一切皆插件"在上下文域的兑现。
2. **模型可见的注入只有一条合法通道**：事件。任何"直接改请求"的捷径都会撞上 L04 的运行时不变量。
3. **视图可以被多种机制改写，但改写要带代数与出处**：replaceGeneration 防竞态覆盖，sourceEventSeqs 保可追溯。
